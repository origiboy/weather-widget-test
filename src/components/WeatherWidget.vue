<template>
    <div class="weather-widget">
        ...
        <div v-if="!loading">
            ...
            <div v-else class="flex add">
                <button 
                    @click="settings = true">Add location</button>
            </div>
        </div>
        <Loader v-else />
...
<script lang="ts">
    ...
    import { getCountryWeather, getCountry } from '../types/interfaces' 

    import { apiGetWeather, apiGetLocation, apiGetCurrentLocation } from '../helpers/api' 
    import { setLocalStorage, getLocalStorage } from '../helpers/tools' 
    ...
    export default defineComponent({
        ...
        data() {
            return {
                dataWeather: {} as getCountryWeather,
                dataCountry: {} as getCountry,
                countryTmp: '' as string,
                countries: [] as string[],
        ...
        methods: {
            getWeather(lat: number, lon: number) {
                apiGetWeather(lat, lon).then((response: any) => {
                    this.dataWeather = response.data
                    this.dataWeather.image = `http://openweathermap.org/img/wn/${this.dataWeather.weather[0].icon}@4x.png`
                    this.countryTmp = `${this.dataCountry.name}, ${this.dataCountry.country}`
                    setLocalStorage('weather-widget-current-location', [this.countryTmp])
                    if (!getLocalStorage('weather-widget-locations').includes(this.countryTmp)) {
                        this.countries.push(this.countryTmp)
                        setLocalStorage('weather-widget-locations', this.countries)
                    }
                    this.loading = false
                })
            },
            getLocation(query: string) {
                this.loading = true
                apiGetLocation(query).then((response: any) => {
                    if (response.data.length === 0) {
                        this.error = "Sorry, we can't get data. Try another city."
            ...
            deleteLocation(query: string) {
                this.loading = true
                const index = this.countries.indexOf(query)
                this.countries.splice(index, 1)
                this.setLocation(this.countries)
                if (this.countries[0]) {
                    this.getLocation(this.countries[0])
                } else {
                    this.countryTmp = ''
                    setLocalStorage('weather-widget-current-location', [this.countryTmp])
                    this.loading = false
            ...
            getCurrentLocation() {
                ...
                if ('geolocation' in navigator) {
                    navigator.geolocation.getCurrentPosition((position) => {
                        apiGetCurrentLocation(position.coords.latitude, position.coords.longitude).then((response: any) => {
                            this.dataCountry = response.data[0]
                            this.getWeather(this.dataCountry.lat, this.dataCountry.lon)
                        })
                    })
            ...
            temperatureFormatted(t: string) :string {
                return Number(t).toFixed(1)
            }
        },
        mounted() {
            const current = getLocalStorage('weather-widget-current-location')
            if (current.length > 0) {
                this.getLocation(current[0])
            } else {
                this.getCurrentLocation()
            }
            this.countries = getLocalStorage('weather-widget-locations')
        },
    })
